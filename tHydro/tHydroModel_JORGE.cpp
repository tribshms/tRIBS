                         ...
                         tWaterBalance *balance, tRunTimer *t)
{
    ...
    // Changes by Giuseppe - August 2016
    stdgrid_opt = infile.ReadItem(stdgrid_opt, "OPTSOILTYPE" ); // Giuseppe - August 2016
    if (stdgrid_opt == 1){
        // JorgeGiuseppe2015 - Start
        int KScount = 0;
        int Thscount = 0;
        int Thrcount = 0;
        int PIcount = 0;
        ...
        SoilGDF(infile); // Giuseppe - August 2016
        SoilResamp(resamp);
    }
    // End of changes by Giuseppe - August 2016
    // JorgeGiuseppe2015 - End
                                     ...
                                     tResample *resamp, int keep)
{
    ...
    //        // Giuseppe - August 2016 --- BEGINNING OF CHANGES
    //        stdgrid_opt = infile.ReadItem(stdgrid_opt, "OPTSOILTYPE" ); // JorgeGiuseppe2015
    //        if (stdgrid_opt == 1){
    //            SoilGDF(infile);
    //        }
    //        // Giuseppe - August 2016 --- END OF CHANGES
...
/***********************JorgeGiuseppe2015***START*************************
 /*************************************************************************
 **
 **  tHydroModel::SoilGDF(tInputFile &, tResample *)
 **
 **  Reads the access path into the soil grid *.gdf file
 **  Initializes the arrays in which the each of the soil parameters
 **  will be stored. The "for" loop goes through all the active nodes
 **  and sets the amount of space required for the arrays.
 **
 *************************************************************************/
void tHydroModel::SoilGDF(tInputFile &infile)
{
    tCNode * cn;
    ...
    int Asize = sID_array;
    sKsat = new double[Asize];  // Surface hydraulic conductivity
    sThs = new double[Asize];  // Saturation moisture content
    sThr = new double[Asize];  // Residual moisture content
    sPoreInd = new double[Asize];  // Pore-size distribution index
    sPsib = new double[Asize];  // Air entry bubbling pressure
    sF = new double[Asize];  // Decay parameter in the exp
    sAr = new double[Asize];  // Anisotropy ratio (saturated)
    sUAr = new double[Asize];  // Anisotropy ratio (unsaturated)
    sporosity = new double[Asize]; // Porosity
    sCond = new double[Asize];  // Conductivity
    sCap = new double[Asize]; // Capacity
    
    soiloptG = infile.ReadItem(soiloptG, "OPTSOILTYPE" );
    if (soiloptG == 1){
        // Read the path to the *.gdf file
        infile.ReadItem(scfile, "SCGRID");
    }
...
/*************************************************************************
 **
 **  tHydroModel::SoilResamp(tResample *resamp)
 **
 **  The resampling of the soil grids is done here. A pointer variable
 **  is assigned for each parameter that will be resampled. It gains
 **  access to the *.gdf file provided by the user. It stores the path
 **  names for each parameter and then resamples each grid file.
 **
 **
 *************************************************************************/
void tHydroModel::SoilResamp(tResample *resamp)
{
    int sID = 0;
    ...
    double *ks_grid_resampled;
    double *ts_grid_resampled;
    double *tr_grid_resampled;
    double *pi_grid_resampled;
    double *pb_grid_resampled;
    double *fd_grid_resampled;
    double *ar_grid_resampled;
    double *ua_grid_resampled;
    double *po_grid_resampled;
    double *vh_grid_resampled;
    double *sh_grid_resampled;
    ...
    nParmSC = numParameters;
    
    //Reads default parameters from the second line of the *.gdf file
    readFile >> SCgridlat;
    readFile >> SCgridlong;
    readFile >> SCgridgmt;
    ...
    for (int ct=0;ct<numParameters;ct++) {
        ...
        cout << "Giuseppe -- Base Name "<< SCgridBaseNames[ct] << "   Ext Name " << SCgridExtNames[ct] << endl;
        ...
        cout << "Giuseppe -- SCgridName[ct] "<< SCgridName[ct] << endl;
    }
    
    
    // Resample Ks grid
    Cout<<"\nResampling Ks grid..."<<endl;
    ks_grid_resampled = resamp->doIt(SCgridName[0],2);
    
    
    // Store Surface hydraulic conductivity
    sID = 0;
    KScount = 0; // Giuseppe
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setKsat(ks_grid_resampled[sID]);
        sID++;
    }
    
    // Resample ThetaS grid
    //Cout<<"\nResampling ThetaS grid..."<<endl;
    ts_grid_resampled = resamp->doIt(SCgridName[1],2);
    sID = 0;
    Thscount = 0; // Giuseppe
    // Store Saturation moisture content
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        //cout << "Lessio - SoilRes - Ths in ID " << cn->getID() << " is " << ts_grid_resampled[sID] << endl; // Giuseppe bug 2016
        setThs(ts_grid_resampled[sID]);
        sID++;
    }
    
    // Resample ThetaR grid
    Cout<<"\nResampling ThetaR grid..."<<endl;
    tr_grid_resampled = resamp->doIt(SCgridName[2],2);
    sID = 0;
    Thrcount = 0; // Giuseppe
    // Store Residual Soil Moisture
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setThr(tr_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Pore Index grid
    Cout<<"\nResampling Pore Index grid..."<<endl;
    pi_grid_resampled = resamp->doIt(SCgridName[3],2);
    sID = 0;
    PIcount = 0; // Giuseppe
    // Store Pore Index
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setPoreInd(pi_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Air E. Bubbling P grid
    Cout<<"\nResampling Air E. Bubbling P grid..."<<endl;
    pb_grid_resampled = resamp->doIt(SCgridName[4],2);
    sID = 0;
    Pbcount = 0; // Giuseppe
    // Store Air E. Bubbling P
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setPsib(pb_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Decay grid
    Cout<<"\nResampling Decay grid..."<<endl;
    fd_grid_resampled = resamp->doIt(SCgridName[5],2);
    sID = 0;
    Fcount = 0; // Giuseppe
    // Store Decay
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setF(fd_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Anisotropy grid
    Cout<<"\nResampling Anisotropy grid..."<<endl;
    ar_grid_resampled = resamp->doIt(SCgridName[6],2);
    sID = 0;
    Arcount = 0; // Giuseppe
    // Store Anisotropy
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setAr(ar_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Unsaturated Anisotropy grid
    Cout<<"\nResampling Unsaturated Anisotropy grid..."<<endl;
    ua_grid_resampled = resamp->doIt(SCgridName[7],2);
    sID = 0;
    UArcount = 0; // Giuseppe
    // Store Unsaturated Anisotropy
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setUAr(ua_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Porosity grid
    Cout<<"\nResampling Porosity grid..."<<endl;;
    po_grid_resampled = resamp->doIt(SCgridName[8],2);
    sID = 0;
    PORcount = 0; // Giuseppe
    // Store Porosity
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setPorosity(po_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Volumetric Heat grid
    Cout<<"\nResampling Volumetric Heat grid..."<<endl;
    vh_grid_resampled = resamp->doIt(SCgridName[9],2);
    sID = 0;
    CONcount = 0; // Giuseppe
    // Store Heat Conductivity
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setCond(vh_grid_resampled[sID]);
        sID++;
    }
    
    // Resample Soil Heat grid
    Cout<<"\nResampling Soil Heat grid..."<<endl;
    sh_grid_resampled = resamp->doIt(SCgridName[10],2);
    sID = 0;
    CAPcount = 0; // Giuseppe
    // Store Heat Capacity
    for( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ){
        setCapac(sh_grid_resampled[sID]);
        sID++;
    }
    
    
    
    delete [] SCgridParamNames;
    delete [] SCgridBaseNames;
    delete [] SCgridExtNames;
    delete [] SCgridName;
...
/***************************************************************************
 **
 ** Set() and Get() Functions for Soil Grid Data
 **
 ***************************************************************************/
void tHydroModel::setKsat(double ks){
    sKsat[KScount] = ks;
    KScount++;
}

double tHydroModel::getKsat(int ksat){
    return sKsat[ksat];
}

void tHydroModel::setThs(double rd){
    sThs[Thscount] = rd;
    Thscount++;
}

double tHydroModel::getThs(int dis){
    return sThs[dis];
}

void tHydroModel::setThr(double rs){
    sThr[Thrcount] = rs;
    Thrcount++;
}

double tHydroModel::getThr(int stor){
    return sThr[stor];
}

void tHydroModel::setPoreInd(double reds){
    sPoreInd[PIcount] = reds;
    PIcount++;
}

double tHydroModel::getPoreInd(int eldis){
    return sPoreInd[eldis];
}

void tHydroModel::setPsib(double rl){
    sPsib[Pbcount] = rl;
    Pbcount++;
}

double tHydroModel::getPsib(int pb){
    return sPsib[pb];
}

void tHydroModel::setF(double sf){
    sF[Fcount] = sf;
    Fcount++;
}

double tHydroModel::getF(int gf){
    return sF[gf];
}

void tHydroModel::setAr(double ar){
    sAr[Arcount] = ar;
    Arcount++;
}

double tHydroModel::getAr(int gar){
    return sAr[gar];
}

void tHydroModel::setUAr(double uar){
    sUAr[UArcount] = uar;
    UArcount++;
}

double tHydroModel::getUAr(int guar){
    return sUAr[guar];
}

void tHydroModel::setPorosity(double por){
    sporosity[PORcount] = por;
    PORcount++;
}

double tHydroModel::getPorosity(int gpor){
    return sporosity[gpor];
}

void tHydroModel::setCond(double cond){
    sCond[CONcount] = cond;
    CONcount++;
}

double tHydroModel::getCond(int gcond){
    return sCond[gcond];
}

void tHydroModel::setCapac(double cap){
    sCap[CAPcount] = cap;
    CAPcount++;
}

double tHydroModel::getCapac(int gcap){
    return sCap[gcap];
}
/**************************************************************************/
/**************************JorgeGiuseppe2015**END**************************/
...
void tHydroModel::InitSet(tResample *resamp)
{
    ...
    //        if (stdgrid_opt == 1){ // JorgeGiuseppe2015
    //            SoilResamp(resamp);
    //        }
    ...
    for (cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP())
    {
        ...
        if (stdgrid_opt == 1){ // JorgeGiuseppe2015
            ...
            Ksat 	= getKsat(cn->getSoilID());  // Surface hydraulic conductivity
            Ths     = getThs(cn->getSoilID());  // Saturation moisture content
            Thr     = getThr(cn->getSoilID());  // Residual moisture content
            PoreInd = getPoreInd(cn->getSoilID());  // Pore-size distribution index
            Psib    = getPsib(cn->getSoilID());  // Air entry bubbling pressure
            F       = getF(cn->getSoilID());  // Decay parameter in the exp
            Ar      = getAr(cn->getSoilID());  // Anisotropy ratio (saturated)
            UAr     = getUAr(cn->getSoilID());  // Anisotropy ratio (unsaturated)
            porosity = getPorosity(cn->getSoilID()); // Porosity
            Eps = 3 + 2/PoreInd;
        }
        else {
            soilPtr->setSoilPtr( cn->getSoilID() ); // Giuseppe Changes 2016
            
            // Giuseppe 2016 - Changes to allow reading soil properties from grids
...
{
    if (stdgrid_opt == 1){ // JorgeGiuseppe2015
        // Get soil hydraulic properties from grids
        ID = cn->getID(); // Giuseppe Changes 2016
        Ksat 	= getKsat(ID);  // Surface hydraulic conductivity
        Ths     = getThs(ID);  // Saturation moisture content
        //cout << "Lessio - ID is " << ID << " -- Ths is = " << Ths << endl; // Giuseppe 2016
        Thr     = getThr(ID);  // Residual moisture content
        PoreInd = getPoreInd(ID);  // Pore-size distribution index
        Psib    = getPsib(ID);  // Air entry bubbling pressure
        F       = getF(ID);  // Decay parameter in the exp
        Ar      = getAr(ID);  // Anisotropy ratio (saturated)
        UAr     = getUAr(ID);  // Anisotropy ratio (unsaturated)
        porosity = getPorosity(ID); // Porosity
        Eps = 3 + 2/PoreInd;
    }
    else {
        soilPtr->setSoilPtr( cn->getSoilID() ); //set current s.type // Giuseppe Changes 2016 - Moved here
...
void tHydroModel::UnSaturatedZone(double dt)
{
    ...
    while ( nodIter.IsActive() ) {
        ...
        R1 = (Ractual + (QpIn-QpOut))*Cos + qrunon;
        R = Ractual;
        ...
        if (SnOpt) {
            ...
            if (snWE > 1e-3)
                R1 = 10*routeWE; //have to convert to mm
        }
        ...
        //cout << "Lessio - Unsaturated - ThSurf is = " << ThSurf << endl; // Giuseppe 2016
        //cout << "Lessio - Unsaturated - Ths is = " << Ths << " in ID " << cn->getID() << endl; // Giuseppe 2016
        if (RuOld == 0.0)
...
void tHydroModel::ComputeFluxesNodes1D()
{
    ...
    for ( cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP() ) {
        ...
        if (stdgrid_opt == 1){ // JorgeGiuseppe2015
            Psib = getPsib(cnorg->getSoilID());  // Air entry bubbling pressure
        }
        else {
        ...
        while ( ce != firstedg || ctr == 0)
        {
            ...
            if ( (cnorg->getBoundaryFlag()  != kOpenBoundary) &&
                ...
                if (stdgrid_opt == 1){ // JorgeGiuseppe2015
                    Psib = getPsib(cndest->getSoilID());  // Air entry bubbling pressure
                }
                else {
                    Psib = soilPtr->getSoilProp(5);
                }
        ...
        if ( WTSlope > 0.0 ) {
            ...
            if (stdgrid_opt == 1){ // JorgeGiuseppe2015
                Ksat 	= getKsat(cnorg->getSoilID());  // Surface hydraulic conductivity
                Ths     = getThs(cnorg->getSoilID());  // Saturation moisture content
                Thr     = getThr(cnorg->getSoilID());  // Residual moisture content
                PoreInd = getPoreInd(cnorg->getSoilID());  // Pore-size distribution index
                Psib    = getPsib(cnorg->getSoilID());  // Air entry bubbling pressure
                F       = getF(cnorg->getSoilID());  // Decay parameter in the exp
                Ar      = getAr(cnorg->getSoilID());  // Anisotropy ratio (saturated)
                UAr     = getUAr(cnorg->getSoilID());  // Anisotropy ratio (unsaturated)
                porosity = getPorosity(cnorg->getSoilID()); // Porosity
                Eps = 3 + 2/PoreInd;
            }
            else {
                Ksat    = soilPtr->getSoilProp(1);  // Surface hydraulic conductivity
