...
//    }
    ...
    else //snow
    {
	  ...
	  snTempC = airTemp;
        ...
        //evaporate liquid from ripe pack
        if (snTempC == 0.0) {
	  //total SWE update
	  ...
	  liqWE += latentHFCalc(resFactCalc())*timeSteps/(rholiqkg*latVapkJ) + 
		cmtonaught*((snUnload + mtoc*rain)*(1 - snowFracCalc()))*timeSteps/3600;
	...
	//sublimate solid from frozen pack
	  ...
	  //liq WE update
	  liqWE += cmtonaught*(snUnload + mtoc*(rain*(1 - snowFracCalc())))*timeSteps/3600;
	...
	//set other fluxes
        ...
        Utot = dUint = iceWE*rholiqkg*cpicekJ*snTempC 
	...
	//change mass (volume) quantities to correct units (kJs)
	  ...
	  Utot = Utotold = iceWE*rholiqkg*cpicekJ*snTempC 
			    + liqWE*rholiqkg*latFreezekJ;
		  ...
		  
	  {
	    ...
	    if (iceWE < 0.1 && iceWE > 0){
	      iceTempC = Usn/(cpicekJ*rholiqkg*0.1);
	    }
	    else
	      iceTempC = Usn/(cpicekJ*rholiqkg*snWE);
	  ...
	  else //melt
	  {
	    ...
	    liqWE += Uwat/(latFreezekJ*rholiqkg); //THM
...
double tSnowPack::sensibleHFCalc(double Kaero)
{
  ...
  //we need to check the units on the airTemp -- Celsius
  shf =  (rhoAir*cpairkJ*Kaero*(airTemp - snTempC));
...
void tSnowPack::snowEB(int nodeID, tCNode* node)
{
  ...
  //calculate emelt THM 2012
  if (Utot > 0)
    emelt=-334*1000*(Utot/(latFreezekJ*rholiqkg));
  else
    emelt= 0; 
  ...
  dUint = (H + Rn + L + Prec + G + emelt/3600)*timeSteps;
