                                 ...
                                 tInputFile *input, tResample *resamp)
{
    ...
    //if (stdgrid_opt == 0){            // Comment out later Giuseppe 2016
    ...
    if (stdgrid_opt == 1)
    {
        ...
        for (int ct=0;ct<numParameters;ct++) {
            ...
            if (strcmp(SCgridParamNames[ct],"KS")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[0], 1);     // resamples grid
            ...
            if (strcmp(SCgridParamNames[ct],"TS")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[1], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"TR")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[2], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"PI")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[3], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"PB")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[4], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"FD")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[5], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"AR")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[6], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"UA")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[7], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"PO")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[8], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"VH")==0)
            {
                ...
                double *tmp = resamp->doIt(SCgridName[9], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
            ...
            if (strcmp(SCgridParamNames[ct],"VSH")==0)
            {
                // Resample Soil Heat grid
                cout << "\nResampling Soil Heat Capacity grid..." << endl;
                double *tmp = resamp->doIt(SCgridName[10], 1);     // resamples grid
                tCNode *cn;
                tMeshListIter <tCNode> niter ( mesh->getNodeList() );
                id = 0;
                for (cn=niter.FirstP(); niter.IsActive(); cn=niter.NextP()) {
