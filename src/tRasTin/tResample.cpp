...
/*
					...
					//begin the long-winded downward spiral into different cases.	    
					if (sumInOrOut == 0) {
						//all the way out
						fracOfGrid += 0.0;
						weightedAveSum += 0.0*base->gridIn[i][j]; 
					}
					else if (sumInOrOut == 4) {
						//all the way in
						fracOfGrid += 1.0;
						weightedAveSum += 1.0*base->gridIn[i][j];
					}
					else {
						
						//partway in
						
						//Only one corner is inside		    
						if (sumInOrOut == 1) {
							//find which ONE corner is IN the polygon
							for (k = 0; k < 4; k++) {
								if (InOrOut[k] == 1)
							...
							//find x-coordinate of corner
							if ( k <= 1 )
								cornerX = base->xulcR + i*base->dR;
							else
								cornerX = base->xulcR + i*base->dR + base->dR;
							
							//find y-coordinate of corner
							if (k == 0 || k == 3)
								cornerY = base->yulcR - j*base->dR;
							else
								cornerY = base->yulcR - j*base->dR - base->dR;
							
							//find area of triangle IN the polygon
							//void vCell::findDirectionIntersectionPoint(int flag, 
							...
							if (k == 0) {
								...
								//initialize tempInd* and set up polygon that we find the area of
								tempInd1 = findDirectionIntersectionPoint(2,cornerX,cornerY,VoronX,VoronY, &trapX1, &trapY1, -1);
								tempInd2 = findDirectionIntersectionPoint(1,cornerX,cornerY,VoronX,VoronY, &trapX2, &trapY2, 1);
								
								if (tempInd1 == -1 || tempInd2 == -1) {
									// this tells us that the coordinates being returned are way, way too large
									area1 = 0.0;
								}
									...
									//initialize for looping through polygon
									polyXY_1[0][0] = cornerX;
									polyXY_1[1][0] = cornerY;
									polyXY_1[0][1] = trapX1;
									polyXY_1[1][1] = trapY1;
									
									//case 1 of where to start and what direction to turn in the Voron*[] arrays
									if ( abs(-tempInd1 + tempInd2) > 0) {
										
										polyXY_1[0][2] = VoronX[tempInd1];
										polyXY_1[1][2] = VoronY[tempInd1];
										...
										while (goodness == 0) {
											...
											//go back to beginning of array of vertices
											if (cntr >= nv)
												cntr = 0;
											...
											//add vertex to polyXY_1 array
											polyXY_1[0][cnt] = VoronX[cntr];
											polyXY_1[1][cnt] = VoronY[cntr];
									...
									//finish polygon
									polyXY_1[0][cnt+1] = trapX2;
									polyXY_1[1][cnt+1] = trapY2;
									
									//calculate area
									area1 = polygonArea(polyXY_1,cnt+1);		    
							...
							//corner k == 1 (bottom left)
							else if (k == 1) {
								tempInd1 = findDirectionIntersectionPoint(1,cornerX,cornerY-base->dR,VoronX,VoronY, &trapX1, &trapY1, 1);
								tempInd2 = findDirectionIntersectionPoint(2,cornerX,cornerY-base->dR,VoronX,VoronY, &trapX2, &trapY2, 1);
								...
								if (tempInd1 == -1 || tempInd2 == -1) {
									...
									//intialize polygon structure
									polyXY_1[0][0] = cornerX;
									polyXY_1[1][0] = cornerY;
									polyXY_1[0][1] = trapX1;
									polyXY_1[1][1] = trapY1;
									
									//if the indices come back in the correct order
									if ( abs(-tempInd1 + tempInd2) > 0) {
										...
										while (goodness == 0) {
											...
											//adjust index if we are at the end of the master array
											if (cntr >= nv)
												cntr = 0;
							...
							//if fourth corner (top right)
							else if (k == 3) {
								tempInd1 = findDirectionIntersectionPoint(1,cornerX,cornerY,VoronX,VoronY, &trapX1, &trapY1, -1);
								tempInd2 = findDirectionIntersectionPoint(2,cornerX,cornerY,VoronX,VoronY, &trapX2, &trapY2, -1);
								...
								//no error state
									...
									//vertices are contained in sequence
									if (abs(-tempInd1 + tempInd2) > 0) {
										
										//initialzie for while loop
										polyXY_1[0][2] = VoronX[tempInd1];
										polyXY_1[1][2] = VoronY[tempInd1];
										cnt = 2;
										cntr = tempInd1;
										...
										while (goodness == 0) {
											...
											//adjust cntr for reaching end of array and needing to start over
											if (cntr >= nv)
												cntr = 0;
								...
								//no error
									...
									if ( abs(-tempInd1 + tempInd2) > 0) {
										...
										while (goodness == 0) {
											...
											//allow for going all the way around vertices
											if (cntr >= nv)
												cntr = 0;
									...
									//no corners are contained between starting vertex and intercept
									else {
										cnt = 1;
									}
									...
									//calculate area
									area1 = polygonArea(polyXY_1,cnt+2);		    
									
								}
							}
							
							//find fraction of area of grid cell IN the polygon
							area1 = polygonArea(polyXY_1, cnt + 1);
							fracOfGrid += area1/(base->dR*base->dR);
							weightedAveSum += (area1/(base->dR*base->dR))*base->gridIn[i][j];
							
						}
						
						//two corners are contained
						else if (sumInOrOut == 2) {
							
							//find which two are contained
							kk = 0;
							tempSum = 0;
							for (k = 0; k < 4; k++) {
								if (InOrOut[k] == 1 && tempSum == 0) {
									//first one found
									kk = k;
									tempSum++;
								}
								else if (InOrOut[k] == 1 && tempSum == 1) {
									//second enclosed corner found
									tempSum++;
									break;
								}
							}
							
							//find area of trapezoid IN the polygon
							//k needs to be either 1-2 or 3-4 to do a x-intercept
							
							//find coordinate of upper left corner
							cornerX = base->xulcR + i*base->dR;
							cornerY = base->yulcR - j*base->dR;
							
							//top left, bottom left corners contained
							if ((kk == 0) && (k == 1)) {
								...
								if ( tempInd1 == -1 || tempInd2 == -1 ) {
									...
									//intialize for while-loop
									polyXY_1[0][0] = cornerX;
									polyXY_1[1][0] = cornerY - base->dR;
									polyXY_1[0][1] = trapX1;
									polyXY_1[1][1] = trapY1;
									
									if ( abs(tempInd2 - tempInd1) > 0) {
										//corners are contained b/t start vertex and intersection
									...
									polyXY_1[0][cnt+1] = trapX2;
									polyXY_1[1][cnt+1] = trapY2;
									polyXY_1[0][cnt+2] = cornerX;
									polyXY_1[1][cnt+2] = cornerY;
							...
							//bottom right to top right
								...
								tempInd1 = findDirectionIntersectionPoint(1,cornerX+base->dR,cornerY,VoronX,VoronY, &trapX1, &trapY1, -1);
								tempInd2 = findDirectionIntersectionPoint(1,cornerX+base->dR,cornerY-base->dR,VoronX,VoronY, &trapX2, &trapY2, -1);
								...
								if (tempInd1 == -1 || tempInd2 == -1) {
									...
									//initialize for while-loop
									polyXY_1[0][0] = cornerX + base->dR;
									polyXY_1[1][0] = cornerY;
									polyXY_1[0][1] = trapX1;
									polyXY_1[1][1] = trapY1;
									
									//vertices are include betwen start vertex and end intersection
									if (abs(tempInd2 - tempInd1) > 0) {
									...
									polyXY_1[0][cnt+1] = trapX2;
									polyXY_1[1][cnt+1] = trapY2;
									polyXY_1[0][cnt+2] = cornerX + base->dR;
									polyXY_1[1][cnt+2] = cornerY - base->dR;
							...
							//upper left and upper right
								...
								// go left to right
								tempInd1 = findDirectionIntersectionPoint(2,cornerX,cornerY,VoronX,VoronY, &trapX1, &trapY1, -1);
								tempInd2 = findDirectionIntersectionPoint(2,cornerX+base->dR,cornerY,VoronX,VoronY, &trapX2, &trapY2, -1);
								if (tempInd1 == -1 || tempInd2 == -1) {
									//invalid
									area1 = 0.0;
								}
									...
									//initialize polygon for while-loop
									polyXY_1[0][0] = cornerX;
									polyXY_1[1][0] = cornerY;
									polyXY_1[0][1] = trapX1;
									polyXY_1[1][1] = trapY1;
									
									//vertices b/t start vertex and end-intersection
									if ( abs(tempInd2 - tempInd1) > 0) {
										
										//intialize polygon and ancillary for while-loop
										polyXY_1[0][2] = VoronX[tempInd1];
										polyXY_1[1][2] = VoronY[tempInd1];
										cnt = 2;
										cntr = tempInd1;
							...
							//bottom left, bottom right
								...
								tempInd1 = findDirectionIntersectionPoint(2,cornerX+base->dR,cornerY-base->dR,VoronX,VoronY, &trapX1, &trapY1, 1);
								tempInd2 = findDirectionIntersectionPoint(2,cornerX,cornerY-base->dR,VoronX,VoronY, &trapX2, &trapY2, 1);
								...
								if (tempInd1 == -1 || tempInd2 == -1) {
									...
									//vertices are included b/t start vertex and intersection
									if ( abs(tempInd2 - tempInd1) > 0) {
										
										//intialize polygon and ancillary information
										polyXY_1[0][2] = VoronX[tempInd1];
										polyXY_1[1][2] = VoronY[tempInd1];
										cnt = 2;
										cntr = tempInd1;
										...
										while (goodness == 0) {
											...
											//account for length of polygon array 
											if (cntr >= nv)
												cntr = 0;
							...
							//diagonals inside (0 to 2)
							else if ( (kk == 0) && (k == 2) ){
								
								//initialize tempInd* and set up polygon that we find the area of
								tempInd1 = findDirectionIntersectionPoint(2,cornerX,cornerY,VoronX,VoronY, &trapX1, &trapY1, -1);
								tempInd2 = findDirectionIntersectionPoint(1,cornerX,cornerY,VoronX,VoronY, &trapX2, &trapY2, 1);
								tempInd3 = findDirectionIntersectionPoint(2,cornerX+base->dR,cornerY-base->dR,VoronX,VoronY,
																		  &trapX3, &trapY3, 1);
								tempInd4 = findDirectionIntersectionPoint(1,cornerX+base->dR,cornerY-base->dR,VoronX,VoronY, 
																		  &trapX4, &trapY4, -1);
								
								if (tempInd1 == -1 || tempInd2 == -1 || tempInd3 == -1 || tempInd4 == -1) {
									// this tells us that the coordinates being returned are way, way too large
									area1 = 0.0;
								}
									...
									//reinitialize to lower corner
									polyXY_1[0][cnt+1] = cornerX + base->dR;
									polyXY_1[1][cnt+1] = cornerY - base->dR;
									cnt = cnt + 2;
									
									if (abs(tempInd2 - tempInd3) > 0 ) {
							...
							//diagonals inside (1 to 3)
							else if ( (kk == 0) && (k == 2) ){
								
								//initialize tempInd* and set up polygon that we find the area of
								tempInd1 = findDirectionIntersectionPoint(2,cornerX,cornerY-base->dR,VoronX,VoronY, &trapX1, &trapY1, -1);
								tempInd2 = findDirectionIntersectionPoint(1,cornerX,cornerY-base->dR,VoronX,VoronY, &trapX2, &trapY2, 1);
								tempInd3 = findDirectionIntersectionPoint(2,cornerX+base->dR,cornerY,VoronX,VoronY,
																		  &trapX3, &trapY3, 1);
								tempInd4 = findDirectionIntersectionPoint(1,cornerX+base->dR,cornerY,VoronX,VoronY, 
																		  &trapX4, &trapY4, -1);
								...
								if (tempInd1 == -1 || tempInd2 == -1 || tempInd3 == -1 || tempInd4 == -1) {
									...
									//case 1 of where to start and what direction to turn in the Voron*[] arrays
									if ( abs(tempInd3 - tempInd2) > 0) {
										
										polyXY_1[0][2] = VoronX[tempInd2];
										polyXY_1[1][2] = VoronY[tempInd2];
									...
									//reinitialize to lower corner
									polyXY_1[0][cnt+1] = cornerX + base->dR;
									polyXY_1[1][cnt+1] = cornerY;
									cnt = cnt + 2;
									
									if ( abs(tempInd1 - tempInd4) > 0) {
									...
									//finish polygon
									polyXY_1[0][cnt+1] = trapX1;
									polyXY_1[1][cnt+1] = trapY1;		      
						...
						//three corners are on the inside
						else if (sumInOrOut == 3) {
							
							//find which corner is OUTSIDE!!
							for (k = 0; k < 4; k++) {
								if (InOrOut == 0)
									break;
							}
							
							//find area of square IN the polygon
							
							//upper corner is OUTSIDE
							if (k == 0) {
								tempInd1 = findDirectionIntersectionPoint(1,cornerX+base->dR,cornerY,VoronX,VoronY, &trapX1, &trapY1, 1);
								tempInd2 = findDirectionIntersectionPoint(2,cornerX,cornerY-base->dR,VoronX,VoronY, &trapX2, &trapY2, -1);
								if (tempInd1 == -1 || tempInd2 == -1) {
									...
									//intialize polygon for while loop
									polyXY_1[0][0] = cornerX + base->dR;
									polyXY_1[1][0] = cornerY;
									polyXY_1[0][1] = trapX1;
									polyXY_1[1][1] = trapY1;
									
									//vertices are b/t starting vertex and intersection
									if ( abs(tempInd2 - tempInd1)  > 0) {
									...
									polyXY_1[0][cnt+2] = cornerX;
									polyXY_1[1][cnt+2] = cornerY - base->dR;
									polyXY_1[0][cnt+3] = cornerX + base->dR;
									polyXY_1[1][cnt+3] = cornerY - base->dR;
									
									//calculate area		  
									area1 = polygonArea(polyXY_1,cnt+3);
								}
							...
							//bottom left corner OUTSIDE
								...
								if (tempInd1 == -1 || tempInd2 == -1) {
									...
									//vertices contained b/t start vertex and intersection
									if ( abs(tempInd2 - tempInd1) > 0) {
							...
							//lower right OUTSIDE
								...
								if (tempInd1 == -1 || tempInd2 == -1) {
									...
									//vertices are contained b/t start vertex and intersection
									if ( abs(tempInd2 - tempInd1) > 0) {
			...
			//get to talk w/ rest of class
			//Check if the there were errors calculating grid fractions.  If so then use the value calculated at the centroid (very rare)
			if(fracOfGrid == 0) {
...
*/
		...
		// GMnSKY2008MLE: NOTE THE FOLLOWING OPERATIONAL ENDING 'else if' BRACE
		}//else if
...
{
	if (flag == 1) {
		*X = vorX[i-1]+(coor-vorY[i-1])*(vorX[i]-vorX[i-1])/(vorY[i]-vorY[i-1]);
		*Y = coor;
	}
	else if (flag == 2) {
		*Y = vorY[i-1]+(vorY[i]-vorY[i-1])*(coor-vorX[i-1])/(vorX[i]-vorX[i-1]);
		*X = coor;
	}
	return;
}
...
/*********************************************************************
**
**  void vCell::findDirectionIntersectionPoint()
**
**  For use in the small grid - big polygon resampling (RINEHART 2007)
**
**  flag is 1 x-int, 2 y-int, coord is the cooresponding y,x coordinate,
**  X,Y are the arrays of vertice coordinates, direction is whether we
**  go +x,y or -x,y (up or down) in the MAP (not matrix)
**
**
**	RINEHART 2007 @ NEW MEXICO TECH
*********************************************************************/
/*
int vCell::findDirectionIntersectionPoint(int flag, 
										  double coordX, double coordY,
										  double *vorX, double *vorY,
										  double *X, double *Y,
										  int dir) {
	
	//this finds the (flag = 1) x-intercept or (flag = 2) y-intercept of
	//a point with the polygon in a direction specified by
	//a inputted direction.
	//  The direction is + or - in the complementary direction (x if y-int, y if x-int)
	//
	int tempGood(0), tempCnt(0), index(0), goodness(0), tempCntr(0);
	...
	if (flag == 1) {
		...
		while (goodness == 0) {
			...
			if (tempCnt > nv) {
				//if more loops than vertices, return -1,-1.
				//  This effectively accounts for major screw-ups in the convertToVoronoiFormat() function
				tempCnt = -1;
				break;
			}
			...
			//found the x-intercept location
			if (( (0 < (tempY2 - coordY)) && (0 >= (tempY1 - coordY) ) )||
				( (0 >= (tempY2 - coordY)) && (0 < (tempY1 -coordY) )))  {
				
				tempY3 = (coordY - tempY1);
				if ((fabs(tempY3) < 1e-3 || fabs(tempY2) < 1e-3) ||
					(fabs(tempY3) > 1e+8 || fabs(tempY2) > 1e+8)){
					//there is no area b/t x-intercept and a vertex, or else the area is ridiculously large
					tempCnt = -1;
					break;
				}
				
				//assign values
				*X = tempY3*(tempX2 - tempX1)/((tempY2 - tempY1)) + tempX1;
				*Y = coordY;
				
				//make sure that value is in the correct direction
				if (dir == int((*X - coordX)/fabs(*X - coordX)))
					goodness = 1;
				else {
		...
		while (goodness == 0) {
			...
			if (tempCnt > nv) {
				//if looped for more than a complete cycle, exit
				tempCnt = -1;
				break;
			}
			...
			//found candidate
			if (( (0 < (tempX2 - coordX)) && (0 >= (tempX1 - coordX) ) )||
				( (0 >= (tempX2 - coordX)) && (0 < (tempX1 -coordX) )))  {
				
				//check
				tempX3 = (coordX - tempX1);
				if ((fabs(tempX3) < 1e-3 || fabs(tempX2) < 1e-3) ||
					(fabs(tempX3) > 1e+7 || fabs(tempX2) > 1e+7)){
					//invalid
					tempCnt = -1;
					break;
				}	
				//may be valid
				*Y = tempX3*(tempY2 - tempY1)/((tempX2 - tempX1)) + tempY1;
				*X = coordX;
				
				//check direction
				if (dir == int((*Y - coordY)/fabs(*Y - coordY)))
					//valid
					goodness = 1;
				else {
